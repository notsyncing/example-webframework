package org.example.webframework.lesson5;
...
public class HttpConnection {
    ...
    public void sendFile(Path path) throws IOException {
        final var fileChannel = AsynchronousFileChannel.open(path, StandardOpenOption.READ);
        buffer.clear();
        ...
        fileChannel.read(buffer, 0, buffer, new CompletionHandler<>() {
            ...
            public void completed(Integer length, ByteBuffer byteBuffer) {
                ...
                final var readCompletionHandler = this;
                buffer.flip();

                try {
                    socket.write(buffer, buffer, new CompletionHandler<>() {
                        @Override
                        public void completed(Integer integer, ByteBuffer byteBuffer) {
                            if (!buffer.hasRemaining()) {
                                buffer.clear();
                                fileChannel.read(buffer, lastReadPosition, buffer, readCompletionHandler);
                            } else {
    ...
    public CompletableFuture<Void> sendFilePartially(Path path, long start, long end) throws IOException {
        final var cf = new CompletableFuture<Void>();
        final var fileChannel = AsynchronousFileChannel.open(path, StandardOpenOption.READ);
        buffer.clear();

        fileChannel.read(buffer, start, buffer, new CompletionHandler<>() {
            private long lastReadPosition = start;
            ...
            public void completed(Integer length, ByteBuffer byteBuffer) {
                ...
                if (end > 0 && lastReadPosition > end) {
                    buffer.limit(length - (int) (lastReadPosition - end) + 1);
                }
